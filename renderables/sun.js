...
class Sun extends Mesh {
    ...
    static setupGeometry() {
        var size = 12;
        var center = new Vector2(size * 0.5 - 0.5, size * 0.5 - 0.5);
        var position = [];
        var color = [];
        var index = [];
        var aux = new Vector2();
        var c = new Color();
        for (var i = 0, x = 0; x < size; x += 1) {
    ...
    onAnimationTick() {
        var { material: { time }, distance } = Sun;
        var { anchor, position } = this;
        var angle = Math.PI * (1.3 - (time * 1.6));
