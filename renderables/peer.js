...
class Peer extends Object3D {
    ...
    onData(data) {
        ...
        switch (data[0]) {
            ...
            case 1:
                {
                    var view = new Float32Array((new Uint8Array(data.slice(1))).buffer);
                    if (view.length >= 7) {
                        var { position, quaternion } = head;
                    ...
                    if (view.length >= 22) {
                        controllers.forEach((controller, i) => {
                            var { hand, position, quaternion } = controller;
                            var offset = 7 + (i * 8);
