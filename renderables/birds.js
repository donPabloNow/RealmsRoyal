...
class Birds extends InstancedMesh {
    ...
    onAnimationTick({ delta }) {
        var { count, material, radius } = Birds;
        var {
            auxVector: aux,
            anchor,
            animations,
        ...
        var velocities = geometry.getAttribute('velocity');
        for (var i = 0; i < count; i += 1) {
            var velocity = velocities.getX(i);
            animations[i] += delta * (0.5 + (velocity * 0.5)) * 0.1;
            var target = { x: targets.getX(i), y: targets.getY(i), z: targets.getZ(i) };
            if (animations[i] > 1) {
                this.resetBird(i, target);
            } else {
                aux.lerpVectors({ x: origins.getX(i), y: origins.getY(i), z: origins.getZ(i) },
                    target,
                    animations[i]
                );
                if (aux.distanceTo(anchor.position) >= radius * 2) {
                    ...
                    var scale = 0.3 + ((1 - velocities.getX(i)) * 0.5);
    ...
    resetBird(i, origin) {
        ...
        var tints = geometry.getAttribute('tint');
        var velocities = geometry.getAttribute('velocity');
        animations[i] = 0;
        target.set(
